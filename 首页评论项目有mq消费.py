...
class IndexReviewSpider():
    ...
    def callback(self,ch, method, properties, jsondata):
        ...
        try:
            ...
            print(f'任务分配错误:{e}--错误所在行数{e.__traceback__.tb_lineno}')
            logging.error(f'{e},错误所在行数{e.__traceback__.tb_lineno} ')  # 将错误信息打印在控制台中
    ...
    def SaveAtDataDb(self, DataList, item):
        ...
        try:
            ...
            logging.error(f'数据库连接错误!{e},错误所在行数{e.__traceback__.tb_lineno} ')  # 将错误信息打印在控制台中
        try:
            ...
            print(f'{item["ReviewId"]} {item["Site"]} {item["Asin"]}数据库存储失败: {e.__traceback__.tb_lineno}行代码出错! ({type(e).__name__+" "+ str(e.args[1])[2:70]}...)\n {Sql}')
